...
cho một string haystack, và một pattern needle, tìm vị trí đầu tiên trong haystack mà needle xuất
...
- ref:
    ...
    It uses hashing technique and brute force for comparison, and is a good candidate for plagiarism detection.


    TC:
        Worst Case: O(mn) == Brute Force
        Best Case: O(m +n)

        But in the best case, if no hash value of the haystack substring matches with hashNeedle,
        then we don't have to iterate character by character in each window.
        In that case, it will be O(n+m).O(n+m).O(n+m).
        Computing the hash value of haystack and needle will be O(m)O(m)O(m) and for traversing all windows,
        we will have O(n−m)O(n-m)O(n−m) time complexity. And during traversal,
        we are doing constant number of operations, hence,
        in that case, it will be O(n−m+2m)=O(n+m)O(n-m+2m)=O(n+m)O(n−m+2m)=O(n+m).

    SC:
        Space complexity: O(1)O(1)O(1). We are not using any extra space.
"""
